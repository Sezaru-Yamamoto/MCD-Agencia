...
export function QuoteForm() {
  ...
  // Leer servicio desde URL hash al cargar
  useEffect(() => {
    const handleHashChange = () => {
      const hash = window.location.hash;
      if (hash.startsWith('#cotizar')) {
        const params = new URLSearchParams(hash.split('?')[1] || '');
        const servicioFromUrl = params.get('servicio');
        if (servicioFromUrl) {
          // Verificar que el servicio existe
          const serviceExists = SERVICES.some(s => s.id === servicioFromUrl);
          if (serviceExists) {
            // Usar setTimeout para asegurar que el form esté montado
            setTimeout(() => {
              // Actualizar el valor del select directamente en el DOM
              ...
              // Activar animación de pulso después del scroll
              setTimeout(() => {
  ...
  console.log('servicioValue:', servicioValue, 'selectedService:', selectedService);

  // Actualizar estado del servicio seleccionado
  const handleServiceChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    const value = e.target.value;
    console.log('handleServiceChange called with:', value);
    setSelectedService(value);
    setRouteInfo(null); // Resetear ruta cuando cambia servicio
  };
  ...
  const onSubmit = async (data: QuoteFormData) => {
    try {
      ...
      setFormStatus('error');
      setErrorMessage(
        error instanceof Error ? error.message : 'Hubo un problema al enviar tu solicitud'
      );
    ...
    <section id="cotizar" className="section bg-gradient-to-br from-cmyk-black to-cmyk-black">
      <div className="container-custom">
        <div className="text-center max-w-3xl mx-auto mb-12">
          <h2 className="mb-4">Solicita tu cotización sin compromiso</h2>
              ...
              <div className="flex items-start">
                ...
                <div>
                  ...
                  <p className="text-gray-300">
                    Te contactaremos en horario de {CONTACT_INFO.businessHours} hrs.
                    Revisa tu email y WhatsApp.
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-6">
              ...
              <div>
                <label htmlFor="nombre" className="label-field">
                  Nombre completo <span className="text-cmyk-magenta">*</span>
                </label>
                <input
                  {...register('nombre')}
                  type="text"
              ...
              <div>
                <label htmlFor="telefono" className="label-field">
                  Teléfono (10 dígitos) <span className="text-cmyk-magenta">*</span>
                </label>
                <input
                  {...register('telefono')}
                  type="tel"
              ...
              {servicioValue !== 'vallas-moviles' && (
                <div>
                  ...
                  >
                    ...
                    <option value="recolección">Recolección en oficina</option>
            ...
            <div className="space-y-6">
              ...
              <div>
                <label htmlFor="tipoSolicitud" className="label-field">
                  Tipo de solicitud <span className="text-cmyk-magenta">*</span>
                </label>
                <select
              ...
              <div className={`relative transition-transform duration-500 ${highlightService ? 'scale-105' : ''}`}>
                <label htmlFor="servicio" className={`label-field transition-colors duration-300 ${highlightService ? 'text-cmyk-cyan font-bold' : ''}`}>
                  Servicio / Producto <span className="text-cmyk-magenta">*</span>
                </label>
                <select
                  {...register('servicio', {
                    onChange: (e) => {
          ...
          <div className="mt-8 space-y-6">
            <div>
              ...
              <textarea
                ...
                placeholder="Describe tu proyecto, necesidades especiales, o cualquier información adicional que nos ayude a cotizar mejor..."
                disabled={formStatus === 'submitting'}
              />
              {errors.comentarios && (
                ...
                onDrop={(e) => {
                  ...
                  if (file) {
                    ...
                    if (!allowedTypes.includes(file.type)) {
                      alert('Tipo de archivo no permitido. Soportados: PDF, JPG, PNG, DOC, DOCX, XLS, XLSX, ZIP');
                      return;
                    }
              ...
              >
                ...
                {selectedFile ? (
                  ...
                  <div>
                    ...
                    <p className="text-white font-medium mb-0.5 text-sm">Haz clic o arrastra un archivo aquí</p>
                    <p className="text-gray-300 text-xs">
                      PDF, JPG, PNG, DOC, DOCX, XLS, XLSX, ZIP (máx. 10MB)
                    </p>
                    <p className="text-gray-300 text-xs mt-1 italic">
                      Si no tienes el archivo ahora, puedes enviarlo después por WhatsApp o email
                    </p>
                  </div>
                )}
              </div>
