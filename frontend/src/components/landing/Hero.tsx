...
export function Hero() {
  ...
  // Build carousel images: show nothing while loading, fallback only if API returned empty
  const carouselImages: CarouselImage[] = useMemo(() => {
    ...
    // Still loading — return empty so the skeleton shows
    if (slidesLoading) return [];
    return FALLBACK_IMAGES;
  }, [apiSlides, slidesLoading]);
      ...
      <div className="container-custom relative z-10 px-4 sm:px-6 md:px-0">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-12 items-center">
          ...
          <div className="relative mt-8 lg:mt-0">
            {slidesLoading || carouselImages.length === 0 ? (
              <div
                className="w-full rounded-xl sm:rounded-2xl overflow-hidden shadow-xl sm:shadow-2xl bg-neutral-800/60 animate-pulse flex items-center justify-center"
                style={{ height: `${carouselHeight}px`, minHeight: '200px' }}
              >
